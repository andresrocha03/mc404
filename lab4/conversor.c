...
char *lltoa(long number, char *string)
{
    ...
    //printf("number: %lli\n",number);
    while (number > 0) {
        stringaux[i] = (number - number/10) + '0';
        // printf("number10: %lli\n",number%10);
        number = number / 10;
        i++;
    }
...
int main()
{
    ...
    write(STDOUT_FD, resposta1, 40);
    write(STDOUT_FD, resposta2, 40);
    write(STDOUT_FD, resposta3, 40);
    write(STDOUT_FD, hexresult, 40);
    write(STDOUT_FD, oresult, 40);
